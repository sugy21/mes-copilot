...
@router.post(
    "/", response_model=WorkResultResponse, status_code=status.HTTP_201_CREATED
)
def create_work_result(work_result: WorkResultCreate, db: Session = Depends(get_db)):
...
def update_work_result(
    work_result_id: int, work_result: WorkResultUpdate, db: Session = Depends(get_db)
):
